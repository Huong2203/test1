            ...
            if ($type == 'html') {
                ...
                foreach ($data as $key => $val) {
                    if ($val['children'] == []) {
                        $html .=
                            '<li class="nav-item">
                            <a class="nav-link" href="' . route('categories.articles', $val['item']->slug) . '">' . $val['item']->name . '</a>';
                    }
                    if ($val['children'] != []) {
                        $html .= '<li class="nav-item dropdown">
                        <a class="nav-link" href="' . route('categories.articles', $val['item']->slug) . '" role="button" data-toggle="dropdown" aria-haspopup="true"
...
if (!function_exists('frontend_recursive_children')) {
    function frontend_recursive_children($data, $type = 'html')
    {
        ...
        foreach ($data as $key => $val) {
            $html .=
                '
                    <a class="dropdown-item" href="' . route('categories.articles', $val['item']->slug) . '">' . $val['item']->name . '</a>
                ';
        }
        $html .= '
        </div>';
